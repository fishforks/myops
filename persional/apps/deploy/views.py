...
class Jenkins_console_out_view(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['console_path'] = "http://192.168.25.12:8080/job/%s/%s/console" % (self.jobname,self.jobnumber)
        # context['console_path'] = "http://192.168.25.12:8080/job/xm-fsp20-cust/59/console"
        return context
...
class DeployappView(LoginRequiredMixin,View):
    def get(self, request,**kwargs):
        ...
        d = Deploydcos.objects.filter(jobname__exact=self.jobname)
        if d.exists():
            # bug: dcos执行失败也提示成功，或者不算bug，直接跳转查看结果. 没有jsonfile不提示。
            buildstatus = Deploydcos.objects.get(jobname__exact=self.jobname).buildstatus
            if s == 0:
                d.update(status=1)
                res = {'status':1,'buildstatus':buildstatus}
